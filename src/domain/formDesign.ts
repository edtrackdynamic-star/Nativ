export interface FormDesign {
  ...
  theme: 'blue' | 'teal' | 'purple'; layout: 'cards' | 'list'; submitLabel: string;
}
export const defaultFormDesign: FormDesign = { title: 'טופס הבחירה שלי', introduction: '', instructions: 'קראו על הקורסים ודרגו את ההעדפות שלכם בכל מקבץ.', documentUrl: '', coverUrl: '', theme: 'blue', layout: 'cards', submitLabel: 'הגשת הבחירות' }
...
export function parseFormDesign(value: unknown): FormDesign {
  ...
  return { title: text('title',150) || defaultFormDesign.title, introduction: text('introduction',4000), instructions: text('instructions',2000), documentUrl: safeLink(data.documentUrl,true), coverUrl: safeLink(data.coverUrl), theme: data.theme ?? 'blue', layout: data.layout ?? 'cards', submitLabel: text('submitLabel',60) || defaultFormDesign.submitLabel }
}
