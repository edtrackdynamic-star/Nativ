...
import { ClusterAssignmentBoard } from './ClusterAssignmentBoard'
import { useConfirmAction } from './interaction'

type View = 'cluster' | 'course' | 'class' | 'student'
const collator = new Intl.Collator('he', { numeric: true })
const rankLabel = (rank: number | null) => rank === null ? 'ללא דירוג' : `דירוג ${rank}`

export function ResultExplorer({ cycleId, run, courses, catalog, placeEditable = true, workflow, onWorkflow, onPublishedChanged, onShowDelivery }: { cycleId: string; run: AssignmentRun; courses: Course[]; catalog: CycleCatalogSnapshot | null; placeEditable?: boolean; workflow?: WorkflowState; onWorkflow?: (next: WorkflowState) => void; onPublishedChanged?: () => Promise<void>; onShowDelivery?: () => void }) {
  ...
  const closeAction = useRef<() => void>(() => undefined)
  const [view, setView] = useState<View>('cluster')
  const [query, setQuery] = useState('')
  const [clusterId, setClusterId] = useState('')
  const [rosterResult, setRosterResult] = useState<{ cycleId: string; students: StudentRosterEntry[] } | null>(null)
  const [errorResult, setErrorResult] = useState<{ cycleId: string; message: string } | null>(null)
  ...
  const roster = rosterResult?.cycleId === cycleId ? rosterResult.students : null
  const loadError = errorResult?.cycleId === cycleId ? errorResult.message : ''
  ...
  const activeClusterId = clusterId || board?.clusters[0]?.id || ''
  ...
  async function leaveDraft(action: () => void) { if (boardBusy || (hasDraft && !await confirm('העברת התלמיד/ה טרם אושרה. לצאת ולבטל את ההצעה?'))) return; action() }
  function closeExplorer() { void leaveDraft(() => setOpen(false)) }
  function showView(next: View) { viewsMenu.current?.removeAttribute('open'); void leaveDraft(() => setView(next)) }
  useEffect(() => { closeAction.current = closeExplorer })

  useEffect(() => {
    if (!open) return
    let active = true
    void getStudentRoster(cycleId).then(values => { if (active) { setRosterResult({ cycleId, students: values }); setErrorResult(null) } }).catch(() => { if (active) setErrorResult({ cycleId, message: 'רשימת התלמידים לא נטענה. אפשר לנסות שוב.' }) })
    return () => { active = false }
  }, [open, cycleId])
  useEffect(() => {
    if (!open) return
    ...
    const onKey = (event: KeyboardEvent) => { if (event.key === 'Escape') closeAction.current() }
  ...
  return <section className="result-explorer" aria-labelledby="result-explorer-title">
    {confirmation}
    <div className="section-heading compact"><div><h4 id="result-explorer-title">תוצאות השיבוץ</h4><p>{run.publishedAt ? 'בדקו את השיבוץ במקבץ אחד או לפי כיתה ותלמיד.' : 'בדקו את ההצעה במקבץ אחד לפני האישור.'}</p></div><button type="button" className="primary-action" onClick={() => { setRosterResult(null); setErrorResult(null); setView('cluster'); setOpen(true) }}>פתיחת לוח השיבוץ</button></div>
    {open && <div className="result-overlay" role="dialog" aria-modal="true" aria-label="תוצאות השיבוץ" dir="rtl">
      <div className="result-dialog"><div className="result-dialog-head"><div className="result-dialog-title"><h3 title={run.label ?? 'תוצאות השיבוץ'}>{run.label ?? 'תוצאות השיבוץ'}</h3><small>{run.publishedAt ? 'שיבוץ שפורסם' : 'שיבוץ מוצע'} · {run.assignments.length} שיבוצים · {new Date(run.executedAt).toLocaleDateString('he-IL')}</small></div><button ref={closeButton} type="button" className="secondary-action" disabled={boardBusy} onClick={closeExplorer}>סגירה</button></div>
        <div className="result-controls"><label className="result-cluster-picker"><span className="sr-only">מקבץ</span><select aria-label="מקבץ" disabled={boardBusy} value={view === 'cluster' ? activeClusterId : clusterId} onChange={event => { const next = event.target.value; void leaveDraft(() => setClusterId(next)) }}>{view !== 'cluster' && <option value="">כל המקבצים</option>}{board?.clusters.map(cluster => <option key={cluster.id} value={cluster.id}>{cluster.label}</option>)}</select></label><label className="result-search"><span className="sr-only">חיפוש</span><input aria-label="חיפוש תלמיד, כיתה או קורס" value={query} onChange={event => setQuery(event.target.value)} placeholder="חיפוש תלמיד/ה או כיתה" /></label><details ref={viewsMenu} className="result-menu"><summary>{({ cluster: 'לוח מקבץ', class: 'לפי כיתה', student: 'לפי תלמיד', course: 'פרטי קורסים' } as const)[view]} ▾</summary><div><button type="button" disabled={boardBusy} aria-current={view === 'cluster' ? 'page' : undefined} onClick={() => showView('cluster')}>לוח מקבץ</button><button type="button" disabled={boardBusy} aria-current={view === 'class' ? 'page' : undefined} onClick={() => showView('class')}>לפי כיתה</button><button type="button" disabled={boardBusy} aria-current={view === 'student' ? 'page' : undefined} onClick={() => showView('student')}>לפי תלמיד</button><button type="button" disabled={boardBusy} aria-current={view === 'course' ? 'page' : undefined} onClick={() => showView('course')}>פרטי קורסים</button><button type="button" disabled={!board} onClick={() => { viewsMenu.current?.removeAttribute('open'); downloadWorkbook() }}>הורדת Excel</button></div></details>{view === 'cluster' && <details className="result-help"><summary>עזרה ומקרא</summary><div><p>בחרו תלמיד/ה ואז לחצו על כותרת הקורס להעברה. כפתור „בחירות” מציג דירוג ונימוק.</p><div className="cluster-board-legend" aria-label="מקרא דירוגים"><span data-rank="first">בחירה 1</span><span data-rank="second">בחירה 2</span><span data-rank="later">בחירה 3 ומטה</span><span data-rank="unranked">לא דורג</span></div></div></details>}</div>
        ...
        {board && <div className="result-groups">
            ...
            {view === 'cluster' && activeClusterId && <ClusterAssignmentBoard key={activeClusterId} cycleId={cycleId} run={run} workflow={workflow} board={board} roster={roster!} courses={displayCourses} catalog={catalog} clusterId={activeClusterId} query={query} editable={!readOnly && placeEditable && Boolean(workflow) && Boolean(onWorkflow || onPublishedChanged)} onWorkflow={onWorkflow} onPublishedChanged={onPublishedChanged} onShowDelivery={onShowDelivery ? () => void leaveDraft(onShowDelivery) : undefined} onDraftChange={setHasDraft} onBusyChange={setBoardBusy} />}
