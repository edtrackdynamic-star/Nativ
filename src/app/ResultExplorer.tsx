...
export function ResultExplorer({ cycleId, run, courses, catalog, placeEditable = true, workflow, onWorkflow, onPublishedChanged, onShowDelivery }: { cycleId: string; run: AssignmentRun; courses: Course[]; catalog: CycleCatalogSnapshot | null; placeEditable?: boolean; workflow?: WorkflowState; onWorkflow?: (next: WorkflowState) => void; onPublishedChanged?: () => Promise<void>; onShowDelivery?: () => void }) {
  ...
  return <section className="result-explorer" aria-labelledby="result-explorer-title">
    ...
    {open && <div className="result-overlay" role="dialog" aria-modal="true" aria-label="תוצאות השיבוץ" dir="rtl">
      <div className="result-dialog"><div className="result-dialog-head"><div><span className="eyebrow">{run.publishedAt ? 'שיבוץ שפורסם' : 'שיבוץ מוצע'}</span><h3>{run.label ?? 'תוצאות השיבוץ'}</h3><p>{new Date(run.executedAt).toLocaleString('he-IL')} · {run.assignments.length} שיבוצים</p></div><button ref={closeButton} type="button" className="secondary-action" disabled={boardBusy} onClick={closeExplorer}>סגירה</button></div>
        <div className="result-controls"><div role="group" aria-label="הצגת תוצאות"><button type="button" disabled={boardBusy} aria-pressed={view === 'cluster'} onClick={() => void leaveDraft(() => setView('cluster'))}>לוח מקבץ</button><button type="button" disabled={boardBusy} aria-pressed={view === 'class'} onClick={() => void leaveDraft(() => setView('class'))}>לפי כיתה</button><button type="button" disabled={boardBusy} aria-pressed={view === 'student'} onClick={() => void leaveDraft(() => setView('student'))}>לפי תלמיד</button><button type="button" disabled={boardBusy} aria-pressed={view === 'course'} onClick={() => void leaveDraft(() => setView('course'))}>פרטי קורסים</button></div>{view !== 'cluster' && <label>חיפוש<input value={query} onChange={event => setQuery(event.target.value)} placeholder="שם תלמיד, כיתה או קורס" /></label>}<label>מקבץ<select disabled={boardBusy} value={view === 'cluster' ? activeClusterId : clusterId} onChange={event => { const next = event.target.value; void leaveDraft(() => setClusterId(next)) }}>{view !== 'cluster' && <option value="">כל המקבצים</option>}{board?.clusters.map(cluster => <option key={cluster.id} value={cluster.id}>{cluster.label}</option>)}</select></label></div>
