...
export function AccessManagerWorkspace() {
  ...
  return <section className="workspace-card" aria-labelledby="access-title"><div className="workspace-heading"><div><span className="eyebrow">משתמשים ותפקידים</span><h2 id="access-title">איוש תפקידים</h2></div></div><p className="workspace-message" aria-live="polite">{message}</p><nav className="role-navigation" aria-label="סוג משתמש"><button aria-pressed={tab === 'staff'} className={tab === 'staff' ? 'active' : ''} onClick={() => setTab('staff')}>צוות</button><button aria-pressed={tab === 'students'} className={tab === 'students' ? 'active' : ''} onClick={() => setTab('students')}>תלמידים</button></nav><label className="search-field">חיפוש לפי שם או דוא״ל<input value={query} onChange={(event) => setQuery(event.target.value)} /></label>{confirmation}<div className="access-table">{users.filter((user) => (tab === 'students' ? user.coreRole === 'student' : user.coreRole !== 'student') && [user.displayName, user.email].filter(Boolean).join(' ').toLocaleLowerCase().includes(query.trim().toLocaleLowerCase())).map((user) => <article key={user.uid}><div><strong>{user.displayName ?? user.email ?? 'משתמש'}</strong><small>{user.email}</small></div><div className="role-checks"><label><input type="checkbox" checked={user.active} disabled={readOnly || pendingUserId !== null} onChange={() => void toggleActive(user)} />גישה פעילה</label>{user.coreRole !== 'student' && accessRoles.filter((role) => role.id !== 'student').map((role) => <label key={role.id}><input type="checkbox" checked={user.roles.includes(role.id)} disabled={readOnly || pendingUserId !== null} onChange={() => void toggle(user, role.id)} />{role.label}</label>)}</div></article>)}</div></section>
}
