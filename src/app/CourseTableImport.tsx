...
const fields = { label: 'שם הקורס', teachers: 'מורים', minimum: 'מינימום', target: 'יעד', maximum: 'מקסימום', documentUrl: 'קישור למסמך', description: 'תיאור', subjectArea: 'תחום דעת' }
...
export function CourseTableImport({ instructors, onAdd, clusterName, existingCourses }: { instructors: Instructor[]; clusterName: string; existingCourses: Array<{label:string}>; onAdd: (courses: CatalogCourseDraft[]) => void }) {
  ...
  function preview() { try {
    ...
    const next = data.map((line, index): ImportRow => {
      ...
      return { teacherText, unresolved, course: { label, description: get('description'), subjectArea: get('subjectArea'), documentUrl: safeLink(get('documentUrl'), true), instructorIds: [...new Set(ids)], minimum: Number(minimum), target: Number(target), maximum: Number(maximum), repeatPolicy: 'allowed' } }
    })
