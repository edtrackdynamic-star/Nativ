...
export function InstructorWorkspace({ cycleId }: { cycleId: string }) {
  ...
  async function refresh() { try { const loaded = await getInstructorWorkspace(cycleId); setData(loaded); setMessage('הנתונים מעודכנים.') } catch { setMessage('רענון הקורסים נכשל. אפשר לנסות שוב.') } }
