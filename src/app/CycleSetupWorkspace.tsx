...
export function CycleSetupWorkspace({ cycle, onChanged, readOnly = false, schoolName, schoolLogo }: { cycle?: AssignmentCycle; onChanged: (createdId?: string) => Promise<void>; readOnly?: boolean; schoolName?: string; schoolLogo?: string }) {
  ...
  return <section className="workspace-card"><h2>עריכת תהליך הבחירה</h2>{confirmation}<p role="status">{message}</p>
    {locked && <p className="read-only-notice">לאחר פתיחת הבחירה אי אפשר לשנות מקבצים וקורסים. את מועד הסגירה אפשר לעדכן בלשונית עיצוב והוראות כל עוד הבחירה פתוחה.</p>}
    ...
    {tab==='design' && (cycle.status==='draft' || cycle.status==='choice_open') && <div className="deadline-settings"><label>מועד סגירת הטופס (רשות, שעון ישראל)<input type="datetime-local" value={deadlineInput} onChange={event=>setDeadlineInput(event.target.value)} disabled={!deadlineEditable || pending}/></label>{cycle.status==='choice_open' && <button type="button" className="secondary-action" disabled={!deadlineEditable || pending || !deadlineDirty} onClick={()=>void saveDeadline()}>שמירת מועד הסגירה</button>}{cycle.status==='draft' && <p>מועד הסגירה יישמר יחד עם הטופס והקורסים.</p>}{cycle.choiceDeadlineEnabled && cycle.choiceClosesAt && <p>המועד השמור: {formatIsraelDateTime(cycle.choiceClosesAt)}</p>}</div>}
