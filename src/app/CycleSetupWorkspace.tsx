...
export function CycleSetupWorkspace({ cycle, onChanged, readOnly = false }: { cycle?: AssignmentCycle; onChanged: (createdId?: string) => Promise<void>; readOnly?: boolean }) {
  ...
  function updateCluster(index:number,patch:Partial<CatalogClusterDraft>){setClusters(items=>items.map((item,i)=>{if(i!==index)return item;const next={...item,...patch};if(next.eligibleClassIds===undefined)delete next.eligibleClassIds;return next}))}
