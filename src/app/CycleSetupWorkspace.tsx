...
const emptyCourse = (): CourseEdit => ({ label: '', description: '', documentUrl:'', imageUrl:'', meetingPlace:'', subjectArea: '', instructorIds: [], minimum: 0, target: 18, maximum: 22, repeatPolicy: 'allowed' })
  ...
  return <section className="workspace-card"><h2>עריכת תהליך הבחירה</h2>{confirmation}<p role="status">{message}</p>
    ...
    />{clusters.map((cluster,ci)=><article className="setup-cluster" key={ci}>
      ...
      {cluster.courses.map((course,ti)=><section className="setup-course" key={ti}><h4>קורס {ti+1}</h4><div className="setup-grid">
        ...
        <label>מקום מפגש (רשות)<input maxLength={120} value={course.meetingPlace??''} onChange={e=>updateCourse(ci,ti,{meetingPlace:e.target.value})} placeholder="לדוגמה: חדר אמנות"/></label>
