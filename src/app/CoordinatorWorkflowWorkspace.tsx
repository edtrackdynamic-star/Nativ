...
import { readLastCoordinatorStage, saveLastCoordinatorStage } from './navigationMemory'
...
export function CoordinatorWorkflowWorkspace({ cycleId, userId, organizationId, onCycleChanged }: { cycleId: string; userId: string; organizationId: string; onCycleChanged: () => Promise<void> }) {
  const readOnly = useReadOnly()
  const { confirm, confirmation } = useConfirmAction()
  const [selectedStage, setSelectedStage] = useState<CoordinatorStageId | null>(() => readLastCoordinatorStage(userId, organizationId, cycleId))
  ...
  useEffect(() => { let active = true; void loadWorkspace(cycleId).then((loaded) => { if (active) { applyLoaded(loaded); setActiveEvaluationId(null); setMessage('הנתונים מעודכנים.') } }).catch((error: unknown) => { if (active) setMessage(error instanceof Error ? error.message : 'טעינת המחזור נכשלה') }); return () => { active = false } }, [cycleId])
  useEffect(() => { if (cycle && workflow) saveLastCoordinatorStage(userId, organizationId, cycleId, selectedStage ?? coordinatorStageProgress(cycle, workflow, now).current) }, [cycle, workflow, selectedStage, userId, organizationId, cycleId, now])
  ...
  return <section className="workspace-card" aria-labelledby="coordinator-title">
    ...
    {tab === 'actual' && (workflow.assignmentRun?.publishedAt ? <><ResultExplorer cycleId={cycleId} run={workflow.assignmentRun} workflow={workflow} courses={courses} catalog={catalogSnapshot} placeEditable={cycle.status !== 'closed'} onPublishedChanged={async()=>{await refresh();await onCycleChanged()}} onShowDelivery={()=>setSelectedStage('delivery')} /><StudentRoster cycleId={cycleId} courseLabels={courseLabels} clusterLabels={clusterLabels} revision={cycleRevision}/></> : <p>טרם פורסם שיבוץ בפועל. עברו לשיבוץ מוצע כדי לבדוק ולאשר אותו.</p>)}
