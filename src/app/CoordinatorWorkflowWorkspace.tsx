...
function EvaluationEditor({ evaluation, cycleId, clusterLabel, onSaved }: { evaluation: AiEvaluation; cycleId: string; clusterLabel: string; onSaved: (workflow: WorkflowState) => void }) {
  ...
  return <article className="workflow-item evaluation-review" aria-label={`בדיקת העדפה במקבץ ${clusterLabel}`}>
    ...
    {hasCourseReview ? <div className="evaluation-courses">{evaluation.input.courses?.filter((course) => course.rank !== null).sort((left, right) => (left.rank ?? 0) - (right.rank ?? 0)).map((course) => <div className="evaluation-course" key={course.courseId}>
      <div className="evaluation-course-heading"><strong>{course.rank}. {course.label}</strong><span>{({ high: 'גבוהה', medium: 'בינונית', neutral: 'ניטרלית' } as const)[evaluation.raw.coursePriorities?.find((entry) => entry.courseId === course.courseId)?.priority ?? 'neutral']}</span></div>
      <label>עדיפות לקורס<select disabled={readOnly || pending} value={coursePriorities[course.courseId] ?? 'neutral'} onChange={(event) => setCoursePriorities((current) => ({ ...current, [course.courseId]: event.target.value as AiPriority }))}><option value="high">גבוהה</option><option value="medium">בינונית</option><option value="neutral">ניטרלית</option></select></label>
...
export function CoordinatorWorkflowWorkspace({ cycleId, onCycleChanged }: { cycleId: string; onCycleChanged: () => Promise<void> }) {
  ...
  return <section className="workspace-card" aria-labelledby="coordinator-title">
    ...
    <nav className="coordinator-stages" aria-label="שלבי תהליך השיבוץ">{progress.stages.map((stage) => <button key={stage.id} type="button" className={`coordinator-stage ${stage.state} ${tab === stage.id ? 'selected' : ''}`} aria-current={tab === stage.id ? 'step' : undefined} onClick={() => void selectTab(stage.id)}><span className="coordinator-stage-number">{stage.number}</span><span className="coordinator-stage-label">{stage.label}</span><small>{stage.state === 'complete' ? stage.id === 'students' ? 'הבחירה נסגרה' : stage.id === 'proposed' ? 'פורסם' : 'אושר' : stage.state === 'current' ? 'כעת' : stage.state === 'available' ? 'זמין לעיון' : 'בהמשך'}</small></button>)}</nav>
    ...
    {tab === 'proposed' && workflow.assignmentRun?.publishedAt && <p>השיבוץ אושר ופורסם. ניתן לצפות בו בשלב פרסום השיבוץ.</p>}
