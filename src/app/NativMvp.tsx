...
export function NativMvp() {
  ...
  return <section className="mvp-shell" aria-labelledby="mvp-title">
    ...
    {availableAreas.length > 1 && <nav className="role-navigation" aria-label="בחירת סביבת עבודה">{availableAreas.map((role) => <button type="button" className={selectedArea === role ? 'active' : ''} aria-current={selectedArea === role ? 'page' : undefined} key={role} onClick={() => setSelectedArea(role)}>{areaLabels[role]}</button>)}</nav>}
    {session && selectedArea && selectedArea !== 'access_manager' && cycles.length > 1 && <label className="cycle-picker">מחזור<select value={selectedCycleId ?? ''} onChange={(event) => setSelectedCycleId(event.target.value)}>{cycles.map((cycle) => <option value={cycle.id} key={cycle.id}>{cycle.schoolYear} · {cycle.termLabel}</option>)}</select></label>}
    {session && selectedArea === 'placement_coordinator' && selectedCycleId && !creatingCycle && <button type="button" className="secondary-action" onClick={() => { setCreatingCycle(true); setSelectedCycleId(null) }}>הקמת מחזור חדש</button>}
    ...
    {session && selectedArea && (selectedCycleId || ['access_manager', 'course_instructor', 'placement_coordinator'].includes(selectedArea)) && <fieldset className="workspace-boundary" disabled={session.access.accessMode === 'read_only'}>
      {selectedArea === 'placement_coordinator' && !selectedCycleId && <CycleSetupWorkspace onChanged={refreshCycles} />}
      {selectedArea === 'placement_coordinator' && cycles.find((cycle) => cycle.id === selectedCycleId)?.status === 'draft' && <CycleSetupWorkspace cycle={cycles.find((cycle) => cycle.id === selectedCycleId)} onChanged={refreshCycles} />}
      {selectedArea === 'student' && selectedCycleId && <StudentPreferenceWorkspace cycleId={selectedCycleId} />}
      {selectedArea === 'placement_coordinator' && selectedCycleId && <CoordinatorWorkflowWorkspace cycleId={selectedCycleId} />}
