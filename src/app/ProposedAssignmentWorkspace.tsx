...
export function ProposedAssignmentWorkspace({ cycleId, workflow, courses, catalog, clusterLabels, readOnly, placeEditable, pending, rejectionReason, onRejectionReason, onWorkflow, onApprove, onPublish, onReject }: {
  cycleId: string; workflow: WorkflowState; courses: Course[]; catalog: CycleCatalogSnapshot | null; clusterLabels: Record<string, string>; readOnly: boolean; placeEditable: boolean; pending: boolean; rejectionReason: string; onRejectionReason: (value: string) => void; onWorkflow: (value: WorkflowState) => void; onApprove: () => void; onPublish: () => void; onReject: () => void
}) {
