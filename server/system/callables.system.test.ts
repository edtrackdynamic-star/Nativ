...
describe('Nativ callable system flow', () => {
  ...
  it('runs anonymous AI review, deterministic placement, publication, and a two-step appeal change', async () => {
    ...
    const secretaryUid = (await getAdminAuth(adminApp).getUserByEmail('secretary@nativ.demo')).uid
    await getFirestore(adminApp).doc(`organizations/${demoCycle.organizationId}/accessAssignments/${secretaryUid}`).set({ active: true, roles: ['secretary'] })
    await getFirestore(adminApp).doc(`organizations/${demoCycle.organizationId}/accessAssignments/inactive-secretary`).set({ active: false, roles: ['secretary'] })
    ...
    expect(workflow.notifications).toHaveLength(3)
    expect((await mailEvents()).size).toBe(1)
    const publication = (await mailEvents()).docs[0].data()
    expect(publication.jobs[0].audience).toBe('secretary')
    expect(publication.jobs[0].recipientIds).toEqual([secretaryUid])
    expect(publication.jobs[0].results).toHaveLength(2)
    expect(JSON.stringify(publication)).not.toMatch(/rationale|originalSubmission|aiEvaluation|explanation/)
    ...
    expect(publicationEvents.size).toBe(2)
    expect(publicationEvents.docs.find(doc => doc.data().jobs[0].audience === 'student')?.data().jobs).toHaveLength(1)
    ...
    expect((await mailEvents()).size).toBe(3)
    const changeEvent = (await mailEvents()).docs.find(doc => doc.data().jobs[0].audience === 'secretary' && !doc.data().jobs[0].results)?.data()
